...
src_prepare() {
	...
	local clear_patch_dir="${FILESDIR}/clear-patches"
	local clear_patches=(
		nsswitch
		mkdir-ldconfig
		nonscd
		...
		tune_adaptive_spin
		nostackshrink
		0001-Set-vector-width-and-alignment-to-fix-GCC-AVX-issue
		disable-vectorization-even-more
		0001-Force-ffsll-to-be-64-bytes-aligned
	)

	for cp in ${clear_patches[@]}; do
		eapply "${clear_patch_dir}/${cp}.patch"
	done
